@with_kw struct IZParameter
    ...
    c::SNNFloat = -65
    d::SNNFloat = 2
    C::SNNFloat = 0.897960714285714
    vr::SNNFloat = -65.2261863636364
end

@with_kw mutable struct IZ
    param::IZParameter = IZParameter(a,b,c,d,C,vr)
    N::SNNInt = 1
...
function integrate!(p::IZ, param::IZParameter, dt::SNNFloat)
    ...
    @unpack a, b, c, d, C, vr = param
    @inbounds for i = 1:N
        v[i] += 0.5f0dt * (0.04f0v[i]^2 + 5f0v[i] + 140f0 - u[i] + I[i])/C
        v[i] += 0.5f0dt * (0.04f0v[i]^2 + 5f0v[i] + 140f0 - u[i] + I[i])/C
        u[i] += dt * (a * (b * v[i] - u[i]))
    end
