...
function learning_plot(plot_path::String, model, config)
    ...
    plot!(
        ...
        xlabel = "Time (ms)",
        ylabel = "Weight",
        title = "E1 to E1 and E1 to E2",
        size = (800, 300),
        margin = 5Plots.mm,
    )
    p2 = histogram(
        model.syn.E1_to_E1.W,
        bins = 0.1:0.01:2.0,
        c = :darkblue,
        alpha = 0.5,
        label = "E1 to E1",
    ...
    # %%
    lags, corr11 = compute_covariance_density(
        merge_spiketimes(spiketimes(model.pop.E1)),
        merge_spiketimes(spiketimes(model.pop.E1)),
    )
    lags, corr12 = compute_covariance_density(
        merge_spiketimes(spiketimes(model.pop.E1)),
        merge_spiketimes(spiketimes(model.pop.E2)),
    )
    p = plot(
        lags,
        corr11,
    ...
    # %%
    frs, r, names =
        firing_rate(model.pop, interval = 0s:50ms:500s, pop_average = true, τ = 50ms)
...
function response_plot(recs, models)
    EEs = [
        mean(model.syn.SST1_to_E1.W)*model.pop.SST1.N/(model.pop.SST1.N + model.pop.PV.N) for model in models
    ]
