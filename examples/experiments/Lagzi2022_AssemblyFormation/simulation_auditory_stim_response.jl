...
for n in eachindex(running_exps)
    ...
    Threads.@threads for t in eachindex(exp_config.NSSTs)
        for syn in [:ampa, :nmda]
            ...
            info = (τ = stim_τ, rate = stim_rate, NSST = NSSTs[t], syn = syn, signal = :off)
...
Threads.@threads for n in eachindex(running_exps)
    ...
    Threads.@threads for t in eachindex(exp_config.NSSTs)
        for syn in [:ampa, :nmda]
            ...
            base_model =
                update_model_parameters!(model; exp_config, config = network_config)

            sound_stim =
                deepcopy(SNN.sample_inputs(exp_config.input_strength, sound, interval))
            @unpack rec_interval = exp_config

            # Set model info
            info = (; info..., train = exp_config.train)
            ...
            if exp_config.force ||
               !isfile(get_path(path = path_response, name = "SoundResponse", info = info))
                ...
                clear_records(model)
                TTL, sim_interval = test_sound_response(model, sound_stim; exp_config...)
                save_model(;
                    path = path_response,
                    model = model,
                    name = "SoundResponse",
