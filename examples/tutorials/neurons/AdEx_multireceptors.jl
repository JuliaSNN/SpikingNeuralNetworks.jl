...
E = SNN.Population(SNN.AdExParameter(El=-55mV), N=1, synapse=synapse, spike=SNN.PostSpike())
model = SNN.compose(E)
SNN.sim!(model; duration=0.5second)
SNN.reset_time!(model)

conn = (p =1, μ = 1.0f0,)
st1 = SNN.SpikeTimeParameter([[0.1s]]) |> x-> SNN.Stimulus(x, E, :gaba1, name="stim1"; conn)
st2 = SNN.SpikeTimeParameter([[0.2s]]) |> x-> SNN.Stimulus(x, E, :gaba2, name="stim2"; conn)
st3 = SNN.SpikeTimeParameter([[0.3s]]) |> x-> SNN.Stimulus(x, E, :gaba3, name="stim3"; conn)
