...
stdp_param = SNN.STDPGerstner(A_pre = 5e-2, 
                              A_post = -5e-2, 
                              τpre = 15ms, 
                              τpost = 25ms,
                              Wmax=Inf, 
...
p3 = SNN.stdp_kernel(stdp_param, fill = true, ΔTs = -1002.5:10:1002.5ms, title="Symmetric Structured STDP")

stdp_param = SNN.STDPAntiSymmetric(αpre = 0.0f0, αpost=0, A_x = 1, A_y = 1,Wmax=Inf, Wmin=-Inf)
p4 = SNN.stdp_kernel(stdp_param, fill = true, ΔTs = -1002.5:10:1002.5ms, title="Anti Symmetric Structured STDP")
# SNN.stdp_weight_decorrelated(stdp_param)

plot(p1, p2, p3, p4,  layout = (2,2), size = (900, 800), legend = false, margin=Plots.mm)

## 
stdp_param = SNN.STDPGerstner(A_pre = 1, 
                              A_post = -1, 
                              τpre = 20ms, 
                              τpost = 20ms, 
                              Wmax=Inf, 
