...
EE = SNN.SpikingSynapse(
    ...
    :he;
    conn = (μ = 2, p = 0.02),
    STPParam = SNN.MarkramSTPParameter(),
)
EI = SNN.SpikingSynapse(
    E,
    I,
    :ge;
    conn = (μ = 30, p = 0.02),
...
## Recording setup
# monitor! registers variables for recording. Buffers are pre-allocated just before
# the time loop by _allocate_records!, not here.
#
# Two recording modes are used automatically:
#   Dense: continuous Float32 variables (v, ge, gi, synaptic vars). One contiguous
#          array of shape (N, capacity) written in-place. getvariable returns a
#          zero-copy view over the written columns.
#   COO:   spike events (:fire). Two flat pre-allocated vectors (times_buf, neurons_buf),
#          one entry per spike. Read via spiketimes(). Pass monitor_rate= to set
#          expected firing rate for capacity; default is 20 Hz.
#
# Pass monitor_time=<ms> to set an explicit capacity hint instead of using the sim duration.
...
SNN.monitor!(model.pop, [:fire])           # COO spike event list
SNN.monitor!(model.pop, [:v], sr = 200Hz)  # dense, downsampled to 200 Hz
...
fr, r =
    SNN.firing_rate(model.pop.E, interval, neurons = 1:100, τ = 100ms, pop_average = true)
plot(r, fr)
...
for τ in [1ms, 10ms, 40ms, 100ms]
    fr, r =
        SNN.firing_rate(model.pop.E, interval, neurons = neurons, τ = τ, pop_average = true)
    ...
    plot!(
        ...
        c = :black,
        lw = 2,
        label = "FR τ = $τ ms; mean $(round(mean(fr), digits=2)) Hz",
...
for ΔT in [1ms, 10ms, 40ms, 100ms]
    ...
    plot!(
        ...
        c = :black,
        lw = 2,
        label = "FR ΔT = $ΔT ms; mean $(round(mean(fr[1, interval]), digits=2)) Hz",
        ylabel = "Firing rate (Hz)",
        legend = :topright,
    )
    bins, r = SNN.bin_spiketimes(model.pop.E, interval)
