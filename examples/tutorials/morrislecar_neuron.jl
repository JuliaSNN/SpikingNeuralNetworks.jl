...
I = SNN.CurrentStimulus(E, :I, param=SNN.CurrentNoiseParameter(E.N, I_dist=Normal(100pA, 200pA) ))
SNN.monitor!(E, [:v, :w])
...
quiver(repeat(vs, outer=length(ws)), repeat(ws, inner=length(vs)), quiver=(ds[1,:,:][:], ds[2,:,:][:]), c=:black, arrow=:filled)
