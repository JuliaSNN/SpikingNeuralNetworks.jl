...
function define_network(N, name, istdp)
    ...
    E = SNN.AdEx(
        N = N,
        param = SNN.AdExParameter(Vr = -55mV, At = 1mV, b = 0, a = 0),
        name = "Exc_$name",
    )
    # Define interneurons 
    I = SNN.IF(;
    ...
    I_to_E =
        SNN.SpikingSynapse(I, E, :hi, p = 0.2, μ = 25, param = istdp, name = "I_to_E_$name")
...
for i in eachindex(subnets)
    for j in eachindex(subnets)
        ...
        synapse =
            SNN.SpikingSynapse(subnets[i].pop.E, subnets[j].pop.I, :he, p = 0.2, μ = 2.0)
...
p1 = plot(
    ...
    legend = :topleft,
    title = "Synaptic weight of I to E synapse",
...
p2 = plot(
    ...
    legend = :topleft,
    title = "Firing rate of the excitatory population",
    lw = 4,
)
...
rates, intervals =
    SNN.firing_rate(exc_populations, interval = interval, τ = 50, interpolate = false)
rates = mean.(rates)
