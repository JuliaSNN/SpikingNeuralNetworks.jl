...
@eval SNN.SNNModels begin
    ...
    # @proto struct PoissonRefractoryParameter{R = Float32} 
        ...
        rate::Vector{Float32} = ones(Float32, N)  # Firing rate for each neuron
    ...
    function stimulate!(
        ...
        @unpack rate, ΔT, last_spike = param
...
neuron = SNN.Identity(; param = param, N = 1, name = "Identity Neuron")

# Create a withe noise input current 
stim_param = PoissonRefractoryParameter(N=1, ΔT = 10ms, p=1)
stim = PoissonLayer(neuron, :g; param=current_param)
